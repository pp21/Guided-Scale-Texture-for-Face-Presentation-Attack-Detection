%% Get local guided binary pattern (LGBP) coding image or hist feature of gray scale image
function lgbprst = lgbp(I, IG, lbp_r, lbp_p, mappingtype, wr, eps, mode)
...
%       'i'  for codes image of guided LBP

% Output parameters:
%  lgbprst: local guided binary pattern (LGBP) coding image or hist feature
...
d_I_enhanced = double(I_enhanced);
...
C_gs = I_enhanced(origy:origy+dy,origx:origx+dx);
d_C_gs = double(C_gs);
...
for i = 1:lbp_p
    ...
    if (abs(x - rx) < 1e-6) && (abs(y - ry) < 1e-6)
        ...
        if mod(lbp_p, 2) == 0
            N = I_enhanced(ry:ry+dy,rx:rx+dx);
            D = N >= C_gs;
        else
        ...
        if mod(lbp_p, 2) == 0
            N = w1*d_I_enhanced(fy:fy+dy,fx:fx+dx) + w2*d_I_enhanced(fy:fy+dy,cx:cx+dx) + ...
                w3*d_I_enhanced(cy:cy+dy,fx:fx+dx) + w4*d_I_enhanced(cy:cy+dy,cx:cx+dx);
            N = roundn(N,-4);
            D = N >= d_C_gs;
        else
